...
class NormalSliceSelector:
    def __init__(self, dataset, seed, device=None, img_size=(192,192), n_clusters=200, context=None, candidate_indices=None):
        ...
        self.seed = seed
        # candidate_indices가 주어지면 그 인덱스 집합 안에서만 정상 슬라이스를 뽑는다.
        # (fold별 train 환자로만 제한해서, val/test 환자의 정상 슬라이스가 train 대표 샘플로
        #  섞여 들어가는 걸 막기 위함 - 환자 단위 분리와 별개로 필요한 안전장치)
        self.candidate_indices = set(candidate_indices) if candidate_indices is not None else None
        ...
        for i, lbl in enumerate(self.dataset.label):
            ...
            if target_bvalue is not None:
                ...
                if side.get('b_value') != target_bvalue:
                    continue
            if self.candidate_indices is not None and i not in self.candidate_indices:
                continue
            indices.append(i)
        return indices
